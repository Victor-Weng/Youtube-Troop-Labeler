...
class DatasetSaver:
    ...
    def __init__(self, output_dir: str = 'dataset', shard_max_images: int = 2000, webp_quality: int = 92, delay_frames: int = 2, tracking_region=None):
        ...
        existing = [f for f in os.listdir(self.shard_dir) if f.startswith('data_') and f.endswith('.tar')]
        max_index = -1
    ...
    def _encode_webp(self, image) -> bytes:
        ok, enc = cv2.imencode('.webp', image, [cv2.IMWRITE_WEBP_QUALITY, self.webp_quality])
        if not ok:
            return b''
        return enc.tobytes()
    ...
    def _commit_frame(self, record: Dict):
        ...
        eff_boxes = [b for b in record.get('boxes', []) if b['track_id'] in eff_ids]
        ...
        with open(self.manifest_path, 'a', encoding='utf-8') as f:
            ...
            track_id_list = '|'.join(str(t['track_id']) for t in meta['tracks'])
            f.write(f"{frame_number},{self.current_shard_name},{webp_name};{json_name},{track_id_list},{json_name}\n")
    ...
    def handle_frame(self, frame_number: int, frame, game_active: bool, active_tracks: List, removed_tracks: List):
        ...
        for t in active_tracks:
            ...
            boxes.append({'track_id': t.track_id, 'x': last['x'], 'y': last['y'], 'w': last['w'], 'h': last['h'], 'card_type': t.card_type, 'player': t.player})
        record = {
            'frame_number': frame_number,
