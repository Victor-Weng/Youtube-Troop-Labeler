...
class TroopDetector:
    def __init__(self):
        ...
        self.logger.info(
            "Using detection-based tracking (industry standard for dynamic scenes)")
        ...
        try:
            ...
            if not self.workspace_name:
                self.logger.warning(
                    "WORKSPACE_CARD_DETECTION not set; Roboflow calls may fail.")
    ...
    def _load_card_names(self) -> Dict[int, str]:
        try:
            json_path = os.path.join(os.path.dirname(
                __file__), 'models', 'cards', 'card_names.json')
            if os.path.exists(json_path):
                ...
                self.logger.warning(
                    f"Card names JSON file not found at {json_path}")
    ...
    def detect_hand_cards(self, frame, which="ally"):
        try:
            # Check if Roboflow client is available
            if not self.card_model:
                print(
                    f"WARNING: Roboflow client not initialized - skipping {which} card detection")
                return {}

            result = self.actions.capture_individual_cards(frame, which)
            card_paths = result['cards']
            print(f"Processing {len(card_paths)} {which} card slots...")
            ...
            for i, card_path in enumerate(card_paths):
                print(f"Analyzing {which} card slot {i}...")
                ...
                try:
                    ...
                    print(
                        f"Roboflow API response for {which} slot {i}: {type(results)}")
                except Exception as api_error:
                    print(
                        f"ERROR calling Roboflow API for {which} slot {i}: {api_error}")
                    results = None
                # print("Card detection raw results:", results)  # Debug print

                # Simple prediction parsing
                print("results :")
                print(results)
                predictions = []
                ...
                if predictions:
                    ...
                    coords = config.ALLY_HAND_COORDS if which == "ally" else config.ENEMY_HAND_COORDS
                    x, y, w, h = coords[i]
            ...
            self.logger.error(f"Error in detect_hand_cards for {which}: {e}")
            return {}
    ...
    def detect_arena_color(self, frame: np.ndarray) -> bool:
        ...
        try:
            ...
            self.arena_color = np.mean(
                arena_region, axis=(0, 1)).astype(np.uint8)
    ...
    def detect_motion(self, frame: np.ndarray) -> List[Dict]:
        ...
        tracking_region = frame[track_y:track_y +
        ...
        for contour in contours:
            ...
            bbox_area = w * h
            # Increased from 0.3 to 0.4 - more restrictive
            if bbox_area > 0 and (area / bbox_area) < 0.4:
                continue
    ...
    def update_tracking(self, frame: np.ndarray, detected_objects: List[Dict]) -> List[Dict]:
        ...
        filtered_objects = self._filter_and_prioritize_objects(
            detected_objects)

        # Card-based tracking control - START AT 0, ONLY TRACK WHEN CARDS PLAYED
        cards_played_recently = len(
            self.recently_disappeared_ally) + len(self.recently_disappeared_enemy)

        if config.CARD_BASED_TRACKING:
            if cards_played_recently == 0:
                # No cards played = no tracking allowed
                self.logger.debug(
                    "No cards played recently - no tracking allowed")
                return []
            else:
                # Allow tracking proportional to cards played, but much more restrictive
                max_tracking_slots = min(
                    config.MAX_TRACKED_OBJECTS, cards_played_recently + 1)  # Only +1 buffer
                self.logger.debug(
                    f"Cards played: {cards_played_recently}, allowing up to {max_tracking_slots} tracked objects")
                # Only take the most significant objects (sorted by area)
                filtered_objects = sorted(filtered_objects, key=lambda x: x['area'], reverse=True)[
                    :max_tracking_slots]
        ...
        for obj in filtered_objects:
            ...
            for existing_id, existing_obj in self.tracked_objects.items():
                if existing_obj.get('active', False):
                    ...
                    predicted_center = existing_obj.get(
                        'predicted_center', existing_obj['center'])
            ...
            if obj_info.get('active', False) and obj_id not in matched_detections:
                ...
                if frames_since_seen <= max_missing_frames:
                    ...
                    missing_obj = {
                        ...
                        # Reduced confidence
                        'confidence': max(0.3, obj_info.get('confidence', 1.0) - 0.2),
                        'status': 'predicted'  # Mark as predicted rather than detected
                    }
                    current_objects.append(missing_obj)
                    self.logger.debug(
                        f"Object {obj_id} missing for {frames_since_seen} frames - maintaining with prediction")
                else:
                    # Object has been missing too long - deactivate
                    obj_info['active'] = False
                    self.logger.debug(
                        f"Object {obj_id} missing for {frames_since_seen} frames - deactivating")
        ...
        for obj in detected_objects:
            ...
            if self._is_in_tracking_region(x, y, w, h):
                ...
                distance_from_center = np.sqrt(
                    (center_x - region_center_x)**2 + (center_y - region_center_y)**2)

                # Priority: larger area + closer to center + existing confidence
                priority_score = (
                    area / 1000) + (1000 / (distance_from_center + 1)) + (obj['confidence'] * 100)
    ...
    def process_frame(self, frame: np.ndarray, frame_number: int) -> Tuple[List[Dict], np.ndarray, List[Dict]]:
        ...
        correlated_objects = self._correlate_motion_with_placements(
        ...
        # Add debug overlay
        debug_frame = self._add_debug_overlay(
            frame, detected_objects, tracked_objects, ally_cards, enemy_cards, placement_events)
    ...
    def _add_debug_overlay(self, frame: np.ndarray, detected: List[Dict], tracked: List[Dict],
        ...
        cv2.putText(debug_frame, "Arena", (x, y-10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Draw tracking region
        track_x, track_y, track_w, track_h = config.TRACKING_REGION
        cv2.rectangle(debug_frame, (track_x, track_y),
                      (track_x + track_w, track_y + track_h), (255, 255, 0), 2)
        ...
        # Add statistics
        cards_played = len(self.recently_disappeared_ally) + \
            len(self.recently_disappeared_enemy)
