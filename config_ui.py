...
class CardRegionConfigurator:
    ...
    def load_frame_from_video(self, time_seconds: float = 15.0) -> bool:
        ...
        try:
            ...
            if config.TEST_VIDEO_PATH:
                print(f"Loading frame at {time_seconds}s from local video: {config.TEST_VIDEO_PATH}")
                cap = cv2.VideoCapture(config.TEST_VIDEO_PATH)
                if not cap.isOpened():
                    print(f"Could not open video file: {config.TEST_VIDEO_PATH}")
                    return False
            elif config.YOUTUBE_URLS:
                print(f"Loading frame at {time_seconds}s from YouTube video...")
            ...
            cap.set(cv2.CAP_PROP_POS_MSEC, time_seconds * 1000)  # Convert to milliseconds
    ...
    def draw_regions(self):
        ...
        cv2.rectangle(self.frame, (x, y), (x + w, y + h), self.colors['arena'], 2)
        ...
        cv2.circle(self.frame, (x + w, y + h), 5, self.colors['arena'], -1)  # Bottom-right
        
        # Draw tracking region
        x, y, w, h = self.regions['tracking']
        cv2.rectangle(self.frame, (x, y), (x + w, y + h), self.colors['tracking'], 2)
        ...
        cv2.circle(self.frame, (x, y), 5, self.colors['tracking'], -1)  # Top-left
        cv2.circle(self.frame, (x + w, y + h), 5, self.colors['tracking'], -1)  # Bottom-right
        
        # Draw ally regions
        for i, (x, y, w, h) in enumerate(self.regions['ally']):
            cv2.rectangle(self.frame, (x, y), (x + w, y + h), self.colors['ally'], 2)
        ...
        for i, (x, y, w, h) in enumerate(self.regions['enemy']):
            cv2.rectangle(self.frame, (x, y), (x + w, y + h), self.colors['enemy'], 2)
        ...
        try:
            ...
            for i, line in enumerate(lines):
                if 'ARENA_SAMPLE_REGION = ' in line:
                    updated_lines.append(f"ARENA_SAMPLE_REGION = {self.regions['arena']}  # Center of 720x1280 frame (portrait)\n")
                elif 'TRACKING_REGION = ' in line:
                    updated_lines.append(f"TRACKING_REGION = {self.regions['tracking']}  # (x, y, w, h) Arena region for tracking\n")
    ...
    def run(self, time_seconds: float = 15.0):
        ...
        loaded = False
        attempts = [time_seconds, 10.0, 5.0, 1.0]  # Try different time positions
